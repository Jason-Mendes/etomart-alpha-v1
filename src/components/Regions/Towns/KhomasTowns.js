...
deliveryTime: true
 ...
 const pharmaciesscroll = useRef(null);
  ...
  const scrollRight = (carouselRef) => {
    if (carouselRef.current) {
      ...
      const aboutus = [
        {
            title: "Easy Ordering",
            description: "Our user-friendly platform makes it simple to order your favorite groceries with just a few clicks.",
            image: "/images/1EO1.png"
        },
        {
            title: "Fast Delivery",
            description: "Enjoy lightning-fast delivery times, ensuring your groceries arrive fresh and on time.",
            image: "/images/2FD2.png"
        },
        {
            title: "Customer Support",
            description: "Our dedicated support team is here to help you with any questions or issues, 24/7.",
            image: "/images/3CS3.png"
        }
    ];
    

    const [currentIndexau, setCurrentIndexau] = useState(0);
  const containerRefau = useRef(null);
  const extendedAboutus = [...aboutus, ...aboutus, ...aboutus];
  ...
  const handleTransitionEndau = () => {
    if (currentIndexau >= extendedAboutus.length - aboutus.length) {
      setCurrentIndexau(aboutus.length);
      containerRefau.current.style.transition = 'none';
      containerRefau.current.style.transform = `translateX(-${aboutus.length * 576}px)`;
      setTimeout(() => {
        containerRefau.current.style.transition = 'transform 0.5s ease-in-out';
      }, 50);
    }
    if (currentIndexau <= 0) {
      setCurrentIndexau(extendedAboutus.length - 2 * aboutus.length);
      containerRefau.current.style.transition = 'none';
      containerRefau.current.style.transform = `translateX(-${(extendedAboutus.length - 2 * aboutus.length) * 576}px)`;
      setTimeout(() => {
        containerRefau.current.style.transition = 'transform 0.5s ease-in-out';
      }, 50);
    }
  ...
  const handleDotClickau = (index) => {
    ...
    return (
      <div>
        ...
        <div className="relative">
          ...
          <div ref={storescards1scroll} className="flex overflow-hidden custom-scrollbar space-x-4 m-4 p-10">
            {storescards1.map((category, shopsindex) => (
              <div key={shopsindex} className="navigationrefrencelink w-52 h-60 flex-shrink-0">
        ...
        {/* storescards1scroll Container */}
        <div className="container mx-auto px-6 m-10">
          <div>
            <main className="my-2">
              <div className="container mx-auto px-4">
                ...
                <div className="relative mt-8 overflow-hidden">
                  <div
                    ...
                    className="flex transition-transform duration-500 ease-in-out"
                    style={{
                      transform: `translateX(-${currentIndexau * 576}px)`,
                  ...
                  >
                    {extendedAboutus.map((aboutus, index) => (
                      <div key={index} className="p-2 flex-shrink-0" style={{ width: '576px', height: '276px' }}>
                        <div className="h-full w-full rounded-md overflow-hidden bg-cover bg-center" style={{ backgroundImage: `url(${aboutus.image})` }}>
                          <div className="bg-gray-900 bg-opacity-50 flex items-center h-full">
                            <div className="px-10 max-w-xl">
                              <h2 className="text-2xl text-white font-semibold">{aboutus.title}</h2>
                              <p className="mt-2 text-gray-400">{aboutus.description}</p>
                              <button className="flex items-center mt-4 text-white text-sm uppercase font-medium rounded hover:underline focus:outline-none">
                                <span>Shop Now</span>
                                <svg className="h-5 w-5 mx-2" fill="none" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewBox="0 0 24 24" stroke="currentColor">
                                  <path d="M17 8l4 4m0 0l-4 4m4-4H3"></path>
                  ...
                  <button className="absolute top-2 right-16 bg-white rounded-full p-2 shadow-md" onClick={handlePrevau}>
                    &lt;
                  </button>
                  <button className="absolute top-2 right-4 bg-white rounded-full p-2 shadow-md" onClick={handleNextau}>
                    &gt;
                  </button>
                  <div className="absolute bottom-4 w-full flex justify-center space-x-2">
                    {aboutus.map((_, index) => (
                      <div
                        key={index}
                        className={`h-2 w-2 rounded-full cursor-pointer ${index === currentIndexau ? 'bg-white' : 'bg-gray-400'}`}
